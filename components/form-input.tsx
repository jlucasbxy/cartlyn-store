  ...
  ({ label, errorMsg, className = "", ...props }, ref) => {
    return (
      <div>
        ...
        <input
          ref={ref}
          className={`appearance-none rounded-md relative block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-white bg-white dark:bg-gray-700 focus:outline-none focus:ring-primary focus:border-primary focus:z-10 text-base transition-colors ${
            errorMsg ? "border-red-500 dark:border-red-500" : ""
          } ${className}`}
          {...props}
        />
        {errorMsg && (
          <p className="mt-1.5 text-base text-red-600 dark:text-red-400">
            {errorMsg}
          </p>
        )}
      </div>
